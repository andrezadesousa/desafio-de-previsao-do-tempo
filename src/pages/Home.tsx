...
export const Home = () => {
  ...
  // Busca case insensitive incluindo altNames
  const filteredCities =
    searchTerm.trim() === ""
      ? cities
      : allCities.filter((city) => {
  ...
  // Função para buscar cidade pelo nome na API de geocoding
  const fetchCityFromApi = async (name: string): Promise<City | null> => {
    try {
      ...
      const c = res.data[0];
      return {
        id: 9999, // id genérico para cidade externa
        name: c.name,
        altNames: [],
        lat: c.lat,
        lon: c.lon,
  ...
  // Quando o usuário confirmar uma pesquisa não listada
  const handleSearchSubmit = async (term: string) => {
    ...
    // Procura primeiro na lista
    const foundInArray = allCities.find(
      (city) =>
        city.name.toLowerCase() === term.toLowerCase() ||
        city.altNames.some((alt) => alt.toLowerCase() === term.toLowerCase())
    );
    if (foundInArray) {
      setSelectedCity(foundInArray);
    } else {
      // Busca pela API
      const cityFromApi = await fetchCityFromApi(term);
      if (cityFromApi) {
